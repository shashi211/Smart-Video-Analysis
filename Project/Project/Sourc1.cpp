...
void ObjectDetection()
{
	...
	VideoCapture cap1("ObjDetection.mp4");
	...
	while (1)
	{
		...
		if (flag == 10 && (getTickCount() / getTickFrequency() - t) >= 2)
		{
			backz = back.clone();
			flag = 20;
		}

		if ((getTickCount() / getTickFrequency() - t) >= interval)
		{
			backz = back.clone();
			t = getTickCount() / (getTickFrequency());
		}
		if ((getTickCount() / getTickFrequency() - t) >= interval - 3)
		{
			cout << "\a";
		}
...
void MaskDetection()
{
	CascadeClassifier mask_cascade1, mask_cascade2;
	VideoCapture cap(0);
	cap.set(CAP_PROP_FRAME_WIDTH, 640);
	cap.set(CAP_PROP_FRAME_HEIGHT, 480);
	Mat img, imgT;
	mask_cascade1.load("cascade/cascade.xml");
	mask_cascade2.load("cascade/cascade_mask2.xml");
	...
	while (1)
	{
		...
		vector<Rect> mask1, mask2;
		mask_cascade1.detectMultiScale(img, mask1, 1.1, 2, 0 | CASCADE_SCALE_IMAGE, Size(30, 30));
		mask_cascade2.detectMultiScale(img, mask2, 1.1, 2, 0 | CASCADE_SCALE_IMAGE, Size(30, 30));
...
void Members()
{
	...
	imshow("-Background-", imgB);
	Mat imgM = imread("imgM.jpg");
	namedWindow("Project Team");
	cvMoveWindow("Project Team", 120, 50);
	cvvResizeWindow("Project Team", 640, 380);

	while (1)
	{
		imshow("Project Team", imgM);
		char c = (char)waitKey(10);
		if (c == 27)
			break;
	}
	destroyWindow("Project Team");
	destroyWindow("-Background-");
}
...
void Windows()
{
	...
	cvvResizeWindow("Members", height, width);
}
...
int main()
{
	while (1)
	{
		...
		setMouseCallback("Members", CallBackFunc4, NULL);
